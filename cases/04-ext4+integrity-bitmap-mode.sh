...
function cmd_up() {
    # Create and open dm-integrity (bitmap mode).
    sudo integritysetup format --integrity sha256 --integrity-bitmap-mode -q "${DISK1_DEV}"
    sudo integritysetup open --integrity sha256 --integrity-bitmap-mode -q "${DISK1_DEV}" "${DISK1_INTEGRITY_NAME}"
