...
function cmd_up() {
    ...
    sudo integritysetup format --integrity sha256 --integrity-no-journal -q "${DISK1_CRYPT_DEV}"
    sudo integritysetup format --integrity sha256 --integrity-no-journal -q "${DISK2_CRYPT_DEV}"
    sudo integritysetup open --integrity sha256 --integrity-no-journal -q "${DISK1_CRYPT_DEV}" "${DISK1_INTEGRITY_NAME}"
    sudo integritysetup open --integrity sha256 --integrity-no-journal -q "${DISK2_CRYPT_DEV}" "${DISK2_INTEGRITY_NAME}"
